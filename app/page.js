...
export default function Home() {
  ...
  const [attResults, setAttResults] = useState({});
  ...
  async function startAttestation(block, resume = false) {
    ...
    if (resume && attProgress[block]) {
      ...
      await supabase.from('attestation_progress').delete().eq('block', block);
    }
  ...
  async function submitAnswer() {
    ...
    if (attQuestionNum < 10) {
      ...
      await supabase.from('attestation_progress').delete().eq('block', attBlock);
      loadAttestationData();
    }
  }
    ...
    <div style={styles.app}>
      ...
      <div style={{position:'relative', zIndex:1, padding:'0 20px 40px'}}>
        ...
        <div style={{marginTop:20, marginBottom:20}}>
          ...
          <div style={{fontSize:30, fontWeight:700, letterSpacing:'-0.5px', color:'#1d1d1f', marginBottom:2}}>CarBase Academy</div>
          <div style={{fontSize:13, color:'#86868b'}}>Платформа обучения менеджеров</div>
        </div>
